...
from digicare.runtime.tumor_diagnosis.models.vit.vit_wrapper import VitWrapper2D, VitWrapper3D
from digicare.runtime.tumor_diagnosis.models.densenet.densenet_wrapper import DenseNetWrapper2D, DenseNetWrapper3D
from digicare.runtime.tumor_diagnosis.models.googlenet.googlenet_wrapper import GoogLeNetWrapper2D
from digicare.runtime.tumor_diagnosis.models.squeezenet.squeezenet_wrapper import SqueezeNetWrapper2D, SqueezeNetWrapper3D
from digicare.runtime.tumor_diagnosis.models.vgg.vgg_wrapper import VGGWrapper2D, VGGWrapper3D
from digicare.runtime.tumor_diagnosis.models.swin_transformer.swin_transformer import SwinTransformerWrapper2D, SwinTransformerWrapper3D
...
def auto_infer_full_configuration_file_path(config:str):
    pattern = 'configs/ablation_study/'+config+'*'
...
def parse_experiment_configuration(args, GLOBAL_CONFIG):
    ...
    def _check_if_experiment_config_is_valid(EXPERIMENT_CONFIG: dict):
        ...
        supported_model_archs = ['custom', 
                                 ...
                                 'densenet201',
                                 'googlenet',
                                 'squeezenet_v1.0', 'squeezenet_v1.1', 'squeezenet_v1.1_gradcam',
    ...
    def _auto_infer_model_output_dir():
        rpath = os.path.relpath(CUSTOM_EXPERIMENT_CONFIGS_PATH, join_path('configs', 'ablation_study'))
...
def process_database_using_custom_rules(GLOBAL_CONFIG, EXPERIMENT_CONFIG):
    ...
    main_database_obj = create_database_for_tumor_diagnosis(MAIN_DATABASE)
    ...
    def _map_class_type_using_custom_rules(database_obj: Database, map_dict):
        ...
        return database_obj
    def _remove_samples_with_invalid_output_class_type(record):
        return True if record[EXPERIMENT_CONFIG['output']] not in EXPERIMENT_CONFIG['output_classes'] else False
    ...
    main_database_obj = main_database_obj.remove_by_rule(_remove_samples_with_invalid_output_class_type)
    ...
    if model_arch == 'custom':
        ...
        if ModelClass_t in [ResidualDownsampleEncoder2d, ResidualDownsampleEncoder3d]:
            model = ModelClass_t(
                ...
                out_classes=len(EXPERIMENT_CONFIG['output_classes']),
        ...
        if input_dim in ['2D', '2.5D']:
            ...
            model = SwinTransformerWrapper3D(
                in_channels=_calculate_input_image_channels(),
                out_classes=len(EXPERIMENT_CONFIG['output_classes']),
                posvec_len=GLOBAL_CONFIG['autoseg_posvec_length'],
                radiomics_vec_len=GLOBAL_CONFIG['radiomics_vec_length'],
        ...
        if input_dim in ['2D', '2.5D']:
            model = GoogLeNetWrapper2D(
                ...
                radiomics_vec_len=GLOBAL_CONFIG['radiomics_vec_length'],
                model_arch=model_arch)
    elif model_arch in ['squeezenet_v1.0', 'squeezenet_v1.1', 'squeezenet_v1.1_gradcam']:
        if input_dim in ['2D', '2.5D']:
            model = SqueezeNetWrapper2D(
                ...
                model_arch=model_arch)
        if input_dim == '3D':
            assert model_arch == 'squeezenet_v1.1', 'only squeezenet_v1.1 supports 3D squeezenet implementation.'
            model = SqueezeNetWrapper3D(
                in_channels=_calculate_input_image_channels(),
                out_classes=len(EXPERIMENT_CONFIG['output_classes']),
                posvec_len=GLOBAL_CONFIG['autoseg_posvec_length'],
                radiomics_vec_len=GLOBAL_CONFIG['radiomics_vec_length'],
                model_arch=model_arch)
    elif model_arch in ['vgg11', 'vgg13', 'vgg16', 'vgg19', 'vgg']:
        if model_arch != 'vgg' and input_dim in ['2D', '2.5D']:
            model = VGGWrapper2D(
                in_channels=_calculate_input_image_channels(),
                out_classes=len(EXPERIMENT_CONFIG['output_classes']),
                posvec_len=GLOBAL_CONFIG['autoseg_posvec_length'],
                radiomics_vec_len=GLOBAL_CONFIG['radiomics_vec_length'],
                model_arch=model_arch)
        elif model_arch == 'vgg' and input_dim == '3D':
            model = VGGWrapper3D(
                in_channels=_calculate_input_image_channels(),
                out_classes=len(EXPERIMENT_CONFIG['output_classes']),
                posvec_len=GLOBAL_CONFIG['autoseg_posvec_length'],
...
def generate_model_optimizer_trainer(GLOBAL_CONFIG, EXPERIMENT_CONFIG, DATA_LOADERS):
    ...
    trainer = GenericClassifierTrainer(
        ...
        all_class_names=EXPERIMENT_CONFIG['output_classes'], class_key_name=EXPERIMENT_CONFIG['output'],
        pretrained_model=EXPERIMENT_CONFIG['pretrained_model'])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='Specify experiment configuration file name (located at "configs/ablation_study/"). '
            'For example, "--experiment-config 001_test" will load experiment configuration from '
            '"configs/ablation_study/001_test.cfg".', 
        required=True)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Specify global configuration file name (located at "configs/ablation_study/"). '
            'For example, "--experiment-config global" will load global configuration from '
            '"configs/ablation_study/global.cfg".', 
        required=False, 
    ...
    if not file_exist(finished_flag):
        ...
        trainer.inference(test_loader=val_loader, enable_tta=False)
