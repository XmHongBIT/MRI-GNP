...
from digicare.utilities.file_ops import file_exist, gd, mkdir
    ...
    * A class that draws things in vector format and saved in PDF files.
    ...
    def parse_unit(s: str) -> float:
        ...
        if 'mm' in s:
            ...
            print('warning: default to cm')
            return float(s) * cm
    @staticmethod
    def parse_position(s: Union[str,tuple,list]) -> Tuple[float, float]:
        '''
        s = "5cm, 3.4cm" or 
        s = "4mm, 1mm"   or 
        s = (5, 3.4). 
    ...
    def __init__(self, output_file:str="output.pdf", pagesize:str="21.0cm*29.7cm", bottomup=True):
        '''
        pagesize:
            example: "210mm * 297mm", "5 in * 5 in", "1920 px * 1080 px"
        bottomup:
            if True, then origin is located at bottom-left corner
            if False, origin is at top-left corner
        
        * space character " " will be ignored
        * 1 pixel = 1/72 inch (72 DPI)
        ...
        self.canvas = canvas.Canvas(self.output_file, pagesize=self.pagesize_in_px, bottomup=bottomup)
    ...
    def text_right(self, s, position, font_name, font_size, font_color=[0,0,0], alpha=1.0, rotation=0.0):
        '''
        * Draw a line of text onto the canvas (right alignment).
    ...
    def text_center(self, s, position, font_name, font_size, font_color=[0,0,0], alpha=1.0, rotation=0.0):
        '''
        * Draw a line of text onto the canvas (center alignment).
    ...
    def circle(self, position_center, radius, line_width, line_color=[0,0,0],
               alpha=1.0, fill_color=[0,0,0]):
        pos = PlotCanvas.parse_position(position_center)
        self.canvas.setLineWidth(line_width)
        self.canvas.setFillColorRGB(fill_color[0],fill_color[1],fill_color[2], alpha = alpha)
...
from digicare.utilities.colormaps import find_colormap_func
import numpy as np
